...
def pubkey(private_key):
    p = run("wg pubkey".split(), input=private_key, stdout=PIPE, universal_newlines=True, check=True)
    return p.stdout.rstrip(os.linesep)

def genpsk():
    p = run("wg genpsk".split(), stdout=PIPE, universal_newlines=True, check=True)
    return p.stdout.rstrip(os.linesep)
